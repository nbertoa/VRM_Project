...
bool UUEOSCReceiver::Connect(FIPv4Address IpAddress, int32 InPort)
{
	UE_LOG(LogTemp, Log, TEXT("[%s]"), *FString(__FUNCTION__));
	if (this->Socket.IsValid() || this->Receiver.IsValid())
	{
	...
	UE_LOG(LogTemp, Log, TEXT("[%s]  FUdpSocketBuilder"), *FString(__FUNCTION__));
	//TSharedPtr<FSocket> NewSocket = MakeShareable(FUdpSocketBuilder(TEXT("UDP Socket")).BoundToEndpoint(endPoint).Build());
	TSharedPtr<FSocket> NewSocket = MakeShareable(FUdpSocketBuilder(TEXT("UDP Socket"))
	...
	UE_LOG(LogTemp, Log, TEXT("[%s]  FUdpSocketReceiver"), *FString(__FUNCTION__));
	TSharedPtr<FUdpSocketReceiver> NewReceiver = MakeShareable(new FUdpSocketReceiver(NewSocket.Get(), FTimespan(0, 0, 1), TEXT("UDP Receiver")));
	if (!NewReceiver.IsValid())
	{
		UE_LOG(LogTemp, Log, TEXT("[%s] Receiver Error"), *FString(__FUNCTION__));
	...
	UE_LOG(LogTemp, Log, TEXT("[%s]  this->Receiver->Start()"), *FString(__FUNCTION__));
