...
namespace {
	...
	void createSmallThumbnail(UVrmAssetListObject *vrmAssetList, const aiScene *mScenePtr) {
		...
		UTexture2D* NewTexture2D = VRMUtil::CreateTexture(W, H, baseName, vrmAssetList->Package);
...
bool VRMConverter::ConvertTextureAndMaterial(UVrmAssetListObject *vrmAssetList, const aiScene *mScenePtr) {
	...
	if (mScenePtr->HasTextures()) {
		...
		for (uint32_t i = 0; i < mScenePtr->mNumTextures; ++i) {
			...
			UTexture2D* NewTexture2D = VRMUtil::CreateTextureFromImage(FString(TEXT("T_")) + baseName, vrmAssetList->Package, t.pcData, t.mWidth);
	...
	if (Options::Get().IsPMXModel()) {
		...
		for (int32_t iMat = 0; iMat < MatNum; ++iMat) {
			...
			FString TextureFullPath = FPaths::GetPath(vrmAssetList->FileFullPathName) + TEXT("/") + ais.C_Str();
			
			FString baseName = ais.C_Str();
			...
			if (FFileHelper::LoadFileToArray(RawFileData, *TextureFullPath)){
				...
				UTexture2D* NewTexture2D = VRMUtil::CreateTextureFromImage(baseName, vrmAssetList->Package, RawFileData.GetData(), RawFileData.Num());
				...
				if (r == AI_SUCCESS) {
					...
					if (Options::Get().IsPMXModel()) {
						for (int i = 0; i < pmxTexNameList.Num(); ++i) {
							if (pmxTexNameList[i] == path.C_Str()) {
								index = i;
								break;
							}
						}
