  ...
  ovrpBoneId_EnumSize = 0x7fff
	...
	*/
	...
	const TArray<VRMUtil::VRMBoneTable> handtable_l_ue4_vrm = {
		...
		{"thumb_03_l","----"},
		{"thumb_01_l","__leftThumbProximal"},
		{"thumb_02_l","__leftThumbIntermediate"},
		{"thumb_03_l","__leftThumbDistal"},
	...
	const TArray<VRMUtil::VRMBoneTable> handtable_r_ue4_vrm = {
		...
		{"lowerarm_r","----"},
		{ "thumb_01_r","__rightThumbProximal" },
		{ "thumb_02_r","__rightThumbIntermediate" },
		{ "thumb_03_r","__rightThumbDistal" },
...
void FAnimNode_VrmQuestHandBone::EvaluateSkeletalControl_AnyThread(FComponentSpacePoseContext& Output, TArray<FBoneTransform>& OutBoneTransforms)
{
	...
	for (int side = 0; side < 2; ++side) {
		for (int boneNo = 0; boneNo < handTable[side]->Num(); ++boneNo) {
			...
			FQuat q;
			if (side == 0) {
				q = FRotator(0, 90, 180).Quaternion();
			} else {
				q = FRotator(0, 90, 0).Quaternion();
			}

			// combine rotator(a,b)  => b.quat * a.quat
			//q = (*handRefTrans[side])[boneNo].GetRotation() * q;
			q = (*handTrans[side])[boneNo].GetRotation() * q;
